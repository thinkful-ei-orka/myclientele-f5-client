...
export default class DashBoardRoute extends React.Component {
  ...
  async componentDidMount() {
    ...
    let activeEmployees = employees.filter(
      (employee) => employee.user_disabled === false
    );
    let inactiveEmployees = employees.filter(
      (employee) => employee.user_disabled === true
    );
  ...
  render() {
    ...
    if (this.state.showingActiveEmployees) {
      if (this.state.inactiveEmployees.length) {
        // show the toggle inactive employees button if showing active employees and there are inactive employees
        toggleActiveButton = (
          <button
            className="btn outline small"
            onClick={(e) => this.toggleActiveEmployees(false)}
          >
      ...
      // show the toggle active employees button if viewing inactive employees
      if (this.state.inactiveEmployees.length === 0) {
        // if there are no inactive employees, switch back to showing active employees
        this.setState({ showingActiveEmployees: true });
      }
      toggleActiveButton = (
        <button
          className="btn small"
          onClick={(e) => this.toggleActiveEmployees(true)}
        >
          ...
          <div className="tooltip_box">
            ...
            <div className="tooltip">
              ...
              <p className="tooltiptext">
                Send this link to your new employees so they can make a new
                account!
              </p>
            </div>
          </div>
